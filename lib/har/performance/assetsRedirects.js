...
var redirect = new Set([301, 302, 303, 305, 306, 307]);

function isRedirect(asset) {
  return redirect.has(asset.status);
}
  ...
  processPage: function (page) {
    ...
    return {
      ...
      advice:
        score < 100
          ? 'The page has ' +
            util.plural(offending.length, 'redirect') +
            '. ' +
            ...
            (offending.length > sameDomainAsDocument
              ? util.plural(
                ...
                ' are from other domains, it could be 3rd-party assets doing unnecessary redirects. :('
              : '')
          : ''
    };
  }
