(function (util) {
  ...
  //   * Accept negotiation, where the URL keeps its .jpg / .png extension
  //     and the server answers image/webp or image/avif.
  //
  // We flag <img> elements that are NOT covered by any of those —
  ...
  const MODERN_CONTENT_TYPES = new Set([
    'image/avif',
    'image/webp',
    'image/jxl'
  ]);

  function isServedAsModernFormat(url) {
    return MODERN_CONTENT_TYPES.has(util.getResourceContentType(url));
  }
  ...
  for (let i = 0, len = images.length; i < len; i++) {
    ...
    total++;
    if (
      isServedAsModernFormat(src) ||
      hasModernSourceSibling(img) ||
      hasModernSrcset(img)
    ) {
      continue;
    }
  ...
  if (total > 0 && legacyOnly > 0) {
    ...
    advice =
      ...
      ' (out of ' +
      total +
      ') in JPEG/PNG/GIF without a modern alternative. Wrap them in a <picture> with a <source type="image/avif"> or "image/webp" before the legacy <img>, or serve modern formats from your image pipeline directly. AVIF and WebP usually deliver 25–50% smaller files at the same quality. If you already negotiate on the Accept header, check that the response really carries an image/webp or image/avif Content-Type, and that cross-origin images send Timing-Allow-Origin so the format is visible to measurement tools.';
  }
  ...
  return {
    ...
    description:
      'AVIF and WebP routinely deliver 25–50% smaller files than JPEG and PNG at the same perceived quality, and every browser version still under support understands at least one of them. Ship modern formats either through a <picture> element with <source type="image/avif"> / "image/webp" entries in front of the legacy <img>, or directly from a content-negotiating image pipeline that returns AVIF / WebP when the client accepts it. Images kept on a .jpg / .png URL and negotiated on the Accept header count as modern: we read the delivered Content-Type from Resource Timing rather than guessing from the file extension. https://web.dev/articles/serve-images-webp',
    advice: advice,
    score: score,
    weight: 4,
    severity: 'warn',
