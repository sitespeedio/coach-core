(function () {
  ...
  if (!supported || supported.indexOf('layout-shift') === -1) {
    ...
    for (let entry of observer.takeRecords()) {
      if (entry.hadRecentInput) {
        continue;
      } // Ignore shifts after recent input.
      score += entry.value;
      for (let shift of entry.sources) {
        if (shift.node && shift.node.currentSrc) {
          offending.push(shift.node.currentSrc);
        } else if (shift.node) {
          if (shift.node.className){
            offending.push(shift.node.nodeName + ' ' + shift.node.className + ' ' + shift.node.textContent);
          } else {
            offending.push(shift.node.nodeName + ' ' + shift.node.textContent);
          }
        }
      }
    }
  ...
  return {
    ...
    advice:
      score > 0
        ? 'You have elements that shift. You should manually check the filmstrip or video and check if it will affect the user.'
        : advice,
