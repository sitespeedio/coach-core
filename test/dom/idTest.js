...
test('Verify advice IDs / We should have an ID that matches the file name', async (t) => {
  ...
  for (const entry of entries) {
    ...
    for (const filename of files) {
      ...
      t.truthy(match, `No id field found in ${filename}`);
      t.is(match[1], name, `Mismatch of ID/filename for ${filename}`);
    }
  }
});
