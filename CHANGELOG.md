...
### Fixed
* Shrink the runtime dependency footprint from eight packages to two. `lodash.sortby`, `json-stable-stringify` and the dev-only `bookmarkletify` were declared but referenced nowhere; `lodash.merge`, `lodash.groupby`, `filter-files` and the dev-only `mkdirp` were one-call helpers replaced with small inline equivalents (the new `lib/support/objectMerge.js` is ported from the same helper sitespeed.io uses in place of `lodash.merge`, where it was verified against the original across 23 edge cases). `dependencies` is now just `pagexray`, `third-party-web` and the vendored Wappalyzer engine [#165](https://github.com/sitespeedio/coach-core/pull/165), [#166](https://github.com/sitespeedio/coach-core/pull/166), [#167](https://github.com/sitespeedio/coach-core/pull/167), [#168](https://github.com/sitespeedio/coach-core/pull/168).
* Drop `connect` and `serve-static` from the test fixture web server in favour of a small native `http` / `http2` handler. The fixture only needed to map a request path to a file under `test/http-server` and stream it back, optionally over http2 with the test certs — a Connect middleware pipeline plus `serve-static` was overkill, and was the last reason to depend on a Connect middleware stack at all [#170](https://github.com/sitespeedio/coach-core/pull/170).
* Migrate the test suite from `mocha` + `chai` + `chai-as-promised` to `ava`, the framework sitespeed.io and browsertime already use. All 45 test files are converted: nested `describe` / `it` is flattened into ava's flat `test('Suite / Case', t => ...)` form so the old grouping stays visible in test names; chai's `should` / `expect` assertions are mapped onto the corresponding `t.is`, `t.deepEqual`, `t.truthy`, `t.regex`, `t.true` / `t.false`. Browser-driven tests keep their per-browser runner setup, but as a single module-level `Map` populated in `test.before` and torn down in `test.after.always`, with `test.serial` + `t.timeout(60_000)` per case [#171](https://github.com/sitespeedio/coach-core/pull/171).
* Bump `third-party-web` to 0.29.2 [#174](https://github.com/sitespeedio/coach-core/pull/174).
* Update to ESLint 10 and align with the sitespeed.io ESLint stack: `eslint` 9.17 → 10.4, `eslint-config-prettier` 9.1 → 10.1.8, `eslint-plugin-prettier` 5.2 → 5.5.5. `@eslint/js` and `globals` were silently transitive under v9 and are now declared explicitly. Drop `FlatCompat` in favour of the flat-config shape ESLint 10 expects natively. Six new `eslint:recommended` findings (`no-useless-assignment` and `preserve-caught-error`) are fixed in place: five `let foo = ''` initializers where every code path reassigned before reading become `let foo;`, and a rethrow in the wappalyzer engine grows a `{ cause: error }` so the original error chains through [#175](https://github.com/sitespeedio/coach-core/pull/175).
* Adopt `eslint-plugin-unicorn` 64, the same major sitespeed.io and browsertime are on. The eslint config moves from `.cjs` to `.js` (ESM) on the way — `eslint-plugin-unicorn` v64 is ESM-only and the package was already `type: module`. The unicorn rule set surfaced 354 findings, 276 of them auto-fixed: a sweep toward modern array methods (`for…of` over `.forEach`, `flatMap` over `map().flat()`, `some` over `findIndex` truthy checks), node-native helpers (`Number.parseInt`, `String.fromCodePoint`, `structuredClone`, `String#slice`, `String#replaceAll`, `String.raw`, `includes`), and tidier conditionals (explicit length checks, no nested ternaries, no useless `undefined`). The remaining hand-fixes are small. The config disables the same six unicorn rules sitespeed.io disables, plus three more that don't fit coach-core's situation: `no-null` (the vendored wappalyzer engine deliberately uses `null` to match the upstream data format), `import-style` (destructured `node:*` imports are clearer than forcing default imports), and `prefer-module` scoped to `lib/dom/**` (those files are not modules — they are IIFE fragments that get concatenated into `dist/coach.min.js` and evaluated in the browser via WebDriver `executeScript`) [#176](https://github.com/sitespeedio/coach-core/pull/176).
* The `terser` build script was running with `--compresss` (three s's). Terser silently treated the typo as an unknown flag and skipped compression entirely, so the shipped `coach.min.js` was mangled-only. Correcting it to `--compress` shrinks the bundle from 54,739 to 52,739 bytes — a ~2 KB win on every release with zero behaviour change [#173](https://github.com/sitespeedio/coach-core/pull/173).
* Drop the unrunnable `eslint-check` npm script. It piped `eslint --print-config .eslintrc.js` (the file is now `eslint.config.cjs`) into `eslint-config-prettier-check`, a binary modern `eslint-config-prettier` no longer ships — the script could not run, and removing it removes confusion [#173](https://github.com/sitespeedio/coach-core/pull/173).
* Backfill HAR tests for fourteen previously untested rules: the eleven HAR privacy rules without coverage (`crossOriginEmbedderPolicyHeader`, `crossOriginOpenerPolicyHeader`, `crossOriginResourcePolicyHeader`, `googleReCaptcha`, `mixedContent`, `nelHeader`, `permissionsPolicyHeader`, `reportingEndpointsHeader`, `thirdPartyCookies`, `thirdPartyPrivacy`, `xContentTypeOptionsHeader`), the HAR-level `avoidRenderBlocking`, and the third-party rules in `bestpractice/thirdParty` and `info/thirdParty`. Header-based rules use a clone-and-inject-header pattern; the third-party rules lean on the existing `www.nytimes.com.har` fixture with `firstParty: 'nytimes\\.com'` set so pagexray classifies off-domain assets correctly. Test count rises from 67 to 101 [#172](https://github.com/sitespeedio/coach-core/pull/172).
